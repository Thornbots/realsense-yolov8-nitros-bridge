...
Both streams run at 60 fps (checked 2026-07-29: `depth_module.profile` is `640x480x60`, matching the filename -- an earlier revision of this file capped depth at 30fps and this paragraph described that, but the halving was never actually present in the config being described). `roi_depth_node` drives off `/detections_output` and only caches the latest depth frame (Phase 0 of the CV tracking plan), so it genuinely does sample depth on detection events rather than every depth frame -- but that no longer needs depth's own publish rate lowered, since the node isn't reacting to every depth frame regardless of what rate it arrives at. If the UVC watchdog / "Depth stream start failure" reappears on bandwidth-constrained USB controllers at 60+60, the fix is capping `depth_module.profile` back to `640x480x30` in the yaml, which would then reduce USB bandwidth, not detection-processing load.
